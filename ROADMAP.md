...
Post quantum cryptography across the stack, with keys, signatures, and hashing built on the NIST post quantum standards. Signatures use ML-DSA-65, key exchange uses ML-KEM-768, SLH-DSA is available, hashing uses SHA-3 and SHAKE, and the transport channel uses ChaCha20-Poly1305.

QORUS consensus. A committee is sampled each round to attest to the proposed block, finality is recorded as a single aggregated certificate signed with ML-DSA-65, and the committee is bounded so the work to finalize a block does not grow with the validator set. See [qip-2.md](qip-2.md).
...
Quantova, post quantum infrastructure written to its own specification. Roadmap is directional and subject to change.
